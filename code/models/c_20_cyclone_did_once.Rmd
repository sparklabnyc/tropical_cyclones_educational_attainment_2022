---
title: "Tropical cyclone DID model for only one exposure"
...
```{r,message=FALSE}
math.DID = readRDS(paste0(prepared.math.DID.folder, "math_DID_cyclones_once.rds")) %>% mutate(year=as.factor(year), state=as.factor(state))
rla.DID = readRDS(paste0(prepared.rla.DID.folder, "rla_DID_cyclones_once.rds")) %>% mutate(year=as.factor(year), state=as.factor(state))
```
...
## Filter out counties that had more than one exposure to hurricanes
```{r}
math.DID = math.DID %>% filter(et==0 | et == 1)
rla.DID = rla.DID %>% filter(et == 0 | et == 1)
```
...
cyclone.math.bayes.state.fixed = data.frame(type='bayes',subject='math',
...
cyclone.rla.bayes.state.fixed = data.frame(type='bayes',subject='rla',
...
cyclone.bayes = data.table::rbindlist(list(cyclone.math.bayes.overall,
                                             ...
                                             cyclone.math.bayes.state.fixed,
                                             cyclone.rla.bayes.overall,
                                             cyclone.rla.bayes.state, 
                                             cyclone.rla.bayes.state.fixed), 
                                             fill = TRUE)
...
```{r,message=FALSE}
readr::write_csv(cyclone.bayes,paste0(model.output.folder,'cyclone_did_national_once.csv'))
```
