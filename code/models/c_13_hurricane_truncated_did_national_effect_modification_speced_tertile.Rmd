---
title: "Hurricane truncated DID model national with effect modification; special education tertiles"
...
fml.bayes.random.interaction.state = gcs_mn_all ~  
                f(state_perspeced_tert,et,model="iid", hyper = lg_prior) +
                f(cohort,model="iid", hyper = lg_prior) +
                f(year,model="iid", hyper = lg_prior) +
                perfl + perrl + perell + pernam + perasn + 
                perhsp + perblk + urban + sesall +
...
```{r,message=FALSE}
math.DID = math.DID %>%
  mutate(state_perspeced_tert=as.factor(paste0(state,'_',perspeced_tert)))
...
hurricane.math.bayes.random.interaction.state = data.frame(type='bayes',subject='math',
                                  #tertile=c(1,2,3),
                                  state=mod.math.bayes.random.interaction.state$summary.random$state_perspeced_tert$ID,
                                  est=mod.math.bayes.random.interaction.state$summary.random$state_perspeced_tert$mean,
                                  ll=mod.math.bayes.random.interaction.state$summary.random$state_perspeced_tert$`0.025quant`,
                                  ul=mod.math.bayes.random.interaction.state$summary.random$state_perspeced_tert$`0.975quant`)
...
```{r,message=FALSE}
readr::write_csv(hurricane.bayes,paste0(model.output.folder,'hurricane_truncated_did_national_specialed_tertile_interaction.csv'))
```
