---
title: "Hurricane DID model national with effect modification; college tertile"
...
fml.bayes.fixed.interaction = gcs_mn_all ~ 
                ...
                perfl + perrl + perell + 
                urban + pernam + perblk + perhsp + 
                perecd + lninc50all + povertyall + snapall + single_momall

fml.bayes.fixed.interaction.state = gcs_mn_all ~  
                state*et*college_tert +
                f(cohort,model="iid", hyper = lg_prior) +
                f(year,model="iid", hyper = lg_prior) +
                perfl + perrl + perell + 
                urban + pernam + perblk + perhsp + 
...
hurricane.math.bayes.fixed.interaction.state = data.frame(type='bayes',subject='math',
                                 tertile=c(1,2,3), #Lines 107-108 not running
...
hurricane.rla.bayes.fixed.interaction.state = data.frame(type='bayes',subject='rla', 
                                  tertile=c(1,2,3), #Lines 120-121 not running
...
```{r,message=FALSE}
readr::write_csv(hurricane.bayes,paste0(model.output.folder,'hurricane_did_national_college_tertile_interaction.csv'))
```