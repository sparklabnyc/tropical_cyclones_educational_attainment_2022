---
title: "Hurricane truncated DID model national with effect modification; SES composite tertiles"
...
```{r,message=FALSE}
math.DID = readRDS(paste0(prepared.math.DID.folder, "math_DID_hurricanes_truncated.rds")) %>% mutate(year=as.factor(year), state=as.factor(state))
rla.DID = readRDS(paste0(prepared.rla.DID.folder, "rla_DID_hurricanes_truncated.rds")) %>% mutate(year=as.factor(year), state=as.factor(state))
```
...
fml.bayes.fixed.interaction = gcs_mn_all ~  
                ...
                perfl + perrl + perell + pernam + perhsp + perblk + perasn + 
...
fml.bayes.random.interaction.state = gcs_mn_all ~  
                f(state_sesall_tert,et,model="iid", hyper = lg_prior) +
                f(cohort,model="iid", hyper = lg_prior) +
                f(year,model="iid", hyper = lg_prior) +
                perfl + perrl + perell + pernam + perhsp + perblk + perasn + 
                urban + perspeced + perecd
...
```{r,message=FALSE}
math.DID = math.DID %>%
  mutate(state_sesall_tert=as.factor(paste0(state,'_',sesall_tert)))
...
mod.rla.bayes.random.interaction.state = inla(fml.bayes.fixed.interaction.state, data = rla.DID)
```
...
hurricane.math.bayes.random.interaction.state = data.frame(type='bayes',subject='math',
                                  #tertile=c(1,2,3),
                                  state=mod.math.bayes.random.interaction.state$summary.random$state_sesall_tert$ID,
                                  est=mod.math.bayes.random.interaction.state$summary.random$state_sesall_tert$mean,
                                  ll=mod.math.bayes.random.interaction.state$summary.random$state_sesall_tert$`0.025quant`,
                                  ul=mod.math.bayes.random.interaction.state$summary.random$state_sesall_tert$`0.975quant`)
...
```{r,message=FALSE}
readr::write_csv(hurricane.bayes,paste0(model.output.folder,'hurricane_truncated_did_national_ses_tertile_interaction.csv'))
```
