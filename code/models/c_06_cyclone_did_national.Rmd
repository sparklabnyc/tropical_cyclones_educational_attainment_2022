---
title: "Cyclone DID model national"
...
```{r,message=FALSE}
math.DID = readRDS(paste0(prepared.math.DID.folder, "math_DID_cyclones.rds")) %>% mutate(year=as.factor(year), state=as.factor(state))
rla.DID = readRDS(paste0(prepared.rla.DID.folder, "rla_DID_cyclones.rds")) %>% mutate(year=as.factor(year), state=as.factor(state))
```
...
fml.bayes.overall = gcs_mn_all ~ 
                ...
                pernam + perasn + perhsp + perblk + perasn + 
                perfl + perell + 
                urban + perspeced +  
...
```{r, message=FALSE}
cyclone.math.bayes.overall = data.frame(type='bayes',subject='math',
                                 state=99,
...
cyclone.math.bayes.state = data.frame(type='bayes',subject='math',
...
cyclone.math.bayes.overall.fixed = data.frame(type='bayes',subject='math',
                                 state=99,
                                 est=mod.math.bayes.overall$summary.fixed$mean,
                                 ll=mod.math.bayes.overall$summary.fixed$`0.025quant`,
                                 ul=mod.math.bayes.overall$summary.fixed$`0.975quant`)

cyclone.rla.bayes.overall = data.frame(type='bayes',subject='rla',
                                 state=99,
...
cyclone.rla.bayes.state = data.frame(type='bayes',subject='rla',
...
cyclone.rla.bayes.overall.fixed = data.frame(type='bayes',subject='rla',
                                 state=99,
                                 est=mod.rla.bayes.overall$summary.fixed$mean,
                                 ll=mod.rla.bayes.overall$summary.fixed$`0.025quant`,
                                 ul=mod.rla.bayes.overall$summary.fixed$`0.975quant`)

cyclone.bayes = data.table::rbindlist(list(cyclone.math.bayes.overall,
                                             cyclone.math.bayes.state,
                                             cyclone.math.bayes.overall.fixed,
                                             cyclone.rla.bayes.overall,
                                             cyclone.rla.bayes.state,
                                             cyclone.rla.bayes.overall.fixed))
...
```{r,message=FALSE}
readr::write_csv(cyclone.bayes,paste0(model.output.folder,'cyclones_did_national.csv'))
```