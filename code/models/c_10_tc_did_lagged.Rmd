---
title: "Cyclone lagged DID model national"
...
```{r,message=FALSE}
math.DID = readRDS(paste0(prepared.math.DID.folder, "math_DID_cyclones_lagged.rds")) %>% mutate(year=as.factor(year), state=as.factor(state))
rla.DID = readRDS(paste0(prepared.rla.DID.folder, "rla_DID_cyclones_lagged.rds")) %>% mutate(year=as.factor(year), state=as.factor(state))
```
...
```{r,message=FALSE}
readr::write_csv(cyclone.bayes,paste0(model.output.folder,'cyclones_did_lagged_national.csv'))
```