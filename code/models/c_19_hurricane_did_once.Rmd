---
title: "Hurricane DID model for only one exposure"
...
```{r,message=FALSE}
math.DID = readRDS(paste0(prepared.math.DID.folder, "math_DID_hurricanes_once.rds")) %>% mutate(year=as.factor(year), state=as.factor(state))
rla.DID = readRDS(paste0(prepared.rla.DID.folder, "rla_DID_hurricanes_once.rds")) %>% mutate(year=as.factor(year), state=as.factor(state))
```
...
```{r}
saveRDS(mod.math.bayes.state, paste0(full.model.output.folder, "math_model_state_once.rds"))
saveRDS(mod.math.bayes.overall, paste0(full.model.output.folder, "math_model_national_once.rds"))
```

## Run Bayesian rla models

```{r,message=FALSE}
mod.rla.bayes.overall = inla(fml.bayes.overall, data = rla.DID, control.compute = list(config = TRUE))
mod.rla.bayes.state = inla(fml.bayes.state, data = rla.DID, control.compute = list(config = TRUE))
```
...
```{r}
saveRDS(mod.rla.bayes.state, paste0(full.model.output.folder, "rla_model_state_once.rds"))
saveRDS(mod.rla.bayes.overall, paste0(full.model.output.folder, "rla_model_national_once.rds"))
```
...
```{r,message=FALSE}
readr::write_csv(hurricane.bayes,paste0(model.output.folder,'hurricane_did_national_once.csv'))
```
