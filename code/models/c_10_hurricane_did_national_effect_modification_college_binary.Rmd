---
title: "Hurricane DID model national with effect modification; college binary"
...
fml.bayes.fixed.interaction = gcs_mn_all ~  
                ...
                perfl + perrl + perell + 
                urban + perblk + perhsp + pernam + 
                perecd + lninc50all + povertyall + snapall + single_momall
```

## Run Bayesian math models
```{r,message=FALSE}
mod.math.bayes.fixed.interaction = inla(fml.bayes.fixed.interaction, data = math.DID) #This line isn't running
```
...
```{r,message=FALSE}
readr::write_csv(hurricane.bayes,paste0(model.output.folder,'hurricane_did_national_college_binary_interaction.csv'))
```