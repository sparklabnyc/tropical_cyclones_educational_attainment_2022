...
library(tidyverse)
library(knitr)
cyclone.data <- readRDS('~/Git/tropical_cyclones_educational_attainment_2022/data/tc_exposure/github_processed/tc_annual_county_exposure_treatment_lagged_2008_2019.rds')

#Rename fips to sedacounty, generate state code, and change to numeric variable
cyclone.data <- cyclone.data %>% rename(sedacounty = fips) %>% mutate(state=substr(sedacounty,1,2))
cyclone.data$sedacounty <- as.numeric(cyclone.data$sedacounty)
...
math.DID <- full_join(processed.math, cyclone.data, by = c('sedacounty', 'year')) %>%
  distinct() %>% relocate(state) %>% filter((is.na(subject)!=TRUE))
rla.DID <- full_join(processed.rla, cyclone.data, by = c('sedacounty', 'year')) %>%
  distinct() %>% relocate(state) %>% filter((is.na(subject)!=TRUE))

#Save DID data in the appropriate folder 
saveRDS(math.DID, paste0(prepared.math.DID.folder, "math_DID_cyclones_lagged.rds"))
saveRDS(rla.DID, paste0(prepared.rla.DID.folder, "rla_DID_cyclones_lagged.rds"))
