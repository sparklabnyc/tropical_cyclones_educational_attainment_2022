...
```{r}
results <- read_csv("~/Git/tropical_cyclones_educational_attainment_2022/output/model_output/hurricane_did_lagged_national.csv")
```
...
fp <- ggplot(data=results, aes(x = est, xmin = ll, xmax = ul, y = code)) +
  ...
  xlab("Estimated Average Grade Scores (95% CrI)") + ylab("Covariate") +
...
#Save forest plot
```{r}
pdf(paste0(plot.output.folder,'hurricanes_lagged_math_covariates','.pdf'))
print(fp)
dev.off()
```
