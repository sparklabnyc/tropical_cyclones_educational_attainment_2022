...
#Load exposure data
cyclone.exposure <- readRDS("~/Git/tropical_cyclones_educational_attainment_2022/data/tc_exposure/github_processed/tc_annual_county_exposure_treatment_2008_2019.rds")
...
USA.df.merged <- merge(USA.df,cyclone.exposure,by.x='GEOID',by.y='fips',all.x=TRUE)
...
p = ggplot() +
  geom_polygon(data=subset(USA.df.merged),aes(x=long,y=lat,group=group,fill=exposure),color='black',size=0.001) +
